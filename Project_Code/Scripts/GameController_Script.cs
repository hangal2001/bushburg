...
// Use this for initialization
	...
	void GetInput()
	{
		...
		{
			CropsAndBuffs.Buff debuff;
			debuff.duration = 200;
			
			debuff.name = "Malaria";
			debuff.buffType = Utilities.BuffTypes.AttributeLockNegative;
			debuff.attribute = Utilities.Attributes.Endurance;
			debuff.value = 3;
			debuff.maxDuration = 200;

			if (selectedCitizen != null)
			{
				selectedCitizen.GetComponent<CitizenBehavior>().ApplyBuff (debuff);
			}
		}
			...
			if (Physics.Raycast (ray, out hit))
			{
				...
				if (hit.collider.tag == "Citizen")
				{
					...
					if (selectedCitizen.GetComponent<CitizenBehavior>().canMove)
						...
						Utilities.FloatText (selectedCitizen.transform.position + new Vector3(0,1,0), "busy!", Utilities.TextTypes.Neutral); 
				...
				if (hit.collider.tag == "Caterpillar")
				{
					draggingObject = hit.collider.gameObject;
					draggingObject.GetComponent<CaterpillarBehavior>().Drag ();
				}

				if (hit.collider.tag == "Pest")
				{
					hit.collider.gameObject.transform.parent.gameObject.GetComponent<FarmplotBehavior>().Eradicate ();
				}
		...
		if (Input.GetButtonUp ("Fire1") && draggingObject != null)
		{
			...
			else if (draggingObject.tag == "Caterpillar")
			{
				draggingObject.GetComponent<CaterpillarBehavior>().Drop ();
			}
			draggingObject = null;
		}
	}
