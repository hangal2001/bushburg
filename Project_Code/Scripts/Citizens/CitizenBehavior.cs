...
public class CitizenBehavior : MonoBehaviour 
{
    ...
    void BuffFilter()
    {
		...
		else if (currentBuff.buffType == Utilities.BuffTypes.AttributeLockNegative)
		{
			if (currentAttributes[currentBuff.attribute] > currentBuff.value)
				currentAttributes[currentBuff.attribute] = currentBuff.value;
		}
	...
	public void Assign()
	{
		...
		{
			currentProspect = currentSlot;
			Utilities.FloatText (transform.position + new Vector3(0,1,0), "station full!", Utilities.TextTypes.Neutral); 
		}
	...
	public void FeedCaterpillar()
	{
		float recov = currentAttributes[Utilities.Attributes.Recovery];


		currentAttributes[Utilities.Attributes.Recovery] = Mathf.Min (maxAttributes[Utilities.Attributes.Recovery], recov + .5f);

		float recoveryDifference = currentAttributes[Utilities.Attributes.Recovery] - recov;
		recoveryDifference = Mathf.Round (recoveryDifference*100)/100;
		string floating = "+" + recoveryDifference.ToString () + " rec";
		Utilities.FloatText (transform.position + new Vector3(0,1,0), floating, Utilities.TextTypes.Positive); 
	}
